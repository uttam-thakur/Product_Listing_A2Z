...
function Product() {
  ...
  const [loadingDelete, setLoadingDelete] = useState(null); // Use product ID for tracking delete status
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(
        `https://hostapi-production-15e5.up.railway.app/api/products/${id}/delete`
      );
      fetchProducts();
      setSuccessMessage("Product deleted successfully!");
      setTimeout(() => {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.post(
        `https://hostapi-production-15e5.up.railway.app/api/products/${editProduct.id}/edit`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
    ...
    <ThemeProvider theme={theme}>
      ...
      <Dialog open={openEditDialog} onClose={handleDialogClose}>
        ...
        <DialogActions>
          ...
          <Button onClick={handleUpdate} color="primary" disabled={loadingEdit}>
            {loadingEdit ? <CircularProgress size={24} /> : "Update"}
          </Button>
        </DialogActions>
      </Dialog>
    </ThemeProvider>
